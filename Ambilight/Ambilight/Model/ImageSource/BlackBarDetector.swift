...
struct BlackBarDetector {
    ...
    func getCroping(image: CGImage) -> Cutoffs {
        ...
        for direction in [Direction.UP, .DOWN, .LEFT, .RIGHT] {
            ...
            cutoffs.setDirection((direction.isHorizontal() ? xTest : yTest), direction: direction)
        }
...
struct Cutoffs {
    ...
    mutating func setDirection(_ value: Int, direction: Direction) {
        switch direction {
        case .UP:
            yMax = value
        case .RIGHT:
